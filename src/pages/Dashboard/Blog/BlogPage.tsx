...
import useListArticleViewModel from './useListArticleViewModel';
import useGlobalLoading from '../../../hooks/useGlobalLoading';
import useModalInputBlog from '../../../components/Modal/hooks/useModalInputBlog';
...
const BlogPage: React.FC = () => {
  const {
    ...
    startIndex,
    endIndex,
  } = useListArticleViewModel();

  const { openModal, editModal } = useModalInputBlog();
  ...
  const handleUpdateStatus = async (slug: string) => {
    ...
    const res = await onUpdateStatus(slug);
        ...
        <div className="flex justify-between items-center mt-2">
          <div>
            <span className="text-xs text-primary-content">
              Showing {startIndex + 1}-{endIndex} of {articles?.total} entries
            </span>
          </div>
          <div>
            <Pagination
